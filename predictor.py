...
class Predictor(Registrable):
    ...
    def post_process(self, line: str, maps: Dict[str, str]) -> str:
        _words = []
    ...
    def evaluate(self, model: Model):
        ...
        if self.post_map is not None:
            predictions = [self.post_process(p, m) for p, m in zip(predictions, self.post_map)]
            references = [self.post_process(r, m) for r, m in zip(references, self.post_map)]
